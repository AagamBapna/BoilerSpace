...
export default function GlobalEventBanner() {
    ...
    const fetchBroadcast = async () => {
      try {
        ...
        if (res.data) {
          ...
          if (dismissedIds.includes(res.data.id || res.data._id)) {
            setDismissed(true);
          } else {
        ...
        console.error('Failed to fetch global broadcast:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDismiss = () => {
    if (!broadcast) return;
    const broadcastId = broadcast.id || broadcast._id;
    const dismissedIds = JSON.parse(localStorage.getItem('dismissed_broadcasts') || '[]');
  ...
  // Determine styling based on priorityLevel 
  ...
  return (
    <div className={`w-full py-3 px-4 sm:px-6 relative flex items-center justify-between border-b z-50 ${bgColorClass}`}>
