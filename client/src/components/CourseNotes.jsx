...
export default function CourseNotes({ courseId, courseName, onClose, userId }) {
  ...
  return (
    <div className="background-blur">
      <div className="course-selector" style={{ maxWidth: '700px' }}>
          ...
          {!loading && !error && notes.length > 0 && (
            <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto">
              {notes.map(note => (
                <div
                  key={note._id}
                  className="flex items-start gap-3 p-6 bg-[var(--color-surface-elevated)] rounded-lg border border-[var(--color-border)] hover:border-[var(--color-purdue-gold)]/30 transition-colors"
                >
